#
# Copyright (c) 2010-2013 by Nils Gรถrs <weechatter@arcor.de>
#
...
#
# history:
# 2.3; add: now buffers with free content can be add to blacklist
...
my $bufname = weechat::buffer_get_string($buf_pointer, "localvar_name");

if ( grep /^$bufname$/, @var_blacklist_channels ) {                            # check for blacklist_channels
  return $string;
}
...
weechat::register($prgname, "Nils Gรถrs <weechatter\@arcor.de>", $version,
                  "GPL3", $description, "", "");
