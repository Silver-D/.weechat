...
def notify(data, buffer, date, tags, displayed, highlight, prefix, message):
	...
	sound = 'Ping' if weechat.config_get_plugin('sound') == 'on' else lambda:_
	if weechat.config_get_plugin('show_highlights') == 'on' and int(highlight):
		...
		if weechat.config_get_plugin('show_message_text') == 'on':
			...
			my_prefix = prefix
			act_config = weechat.config_get('weechat.look.prefix_action')
			act_config = weechat.config_string(act_config)
			if prefix == act_config:
				my_prefix = message.split(' ')[0]
			Notifier.notify('In %s by %s' % (channel, my_prefix), title='Highlighted Message', sound=sound)
